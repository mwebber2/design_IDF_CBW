...
def multiplemaps(listname, titletosave):
    ...
    for i in range(len(RP_list)): #6 RPs
        ...
        variable = RP_list[i]
        CBWcounties.plot(column = variable, cmap='Blues', linewidth=0.8, ax=axs[i], edgecolor='0.8', legend = False)
...
for i in range(len(RP_list)): #6 RPs
    ...
    variable = RP_list[i]
    CBWcounties.plot(column = variable, cmap='Blues', linewidth=0.8, ax=axs[i], edgecolor='0.8')#, label = variable)#, legend = True)
    axs[i].set_title(RP_list[i])
    sm = plt.cm.ScalarMappable(cmap='Blues', norm=plt.Normalize(vmin=vmin, vmax=vmax))
    sm._A = [] #empty array for the data range
