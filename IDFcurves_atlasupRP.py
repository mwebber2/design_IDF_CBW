# -*- coding: utf-8 -*-
"""
Created on Sun May 19 15:23:12 2024
...
for c in range(len(county_centroids)):
    ...
    for r in range(len(r_master)-1): #for each RP -1 because 100yr has no up RP #TODO  
        diff_min = (atlas_median[r+1] - min(r_master[r]))/atlas_median[r+1]
        ...
        diff_max = (atlas_median[r+1] - max(r_master[r]))/atlas_median[r+1]
        ...
        diff_45short = (atlas_median[r+1] - r_master[r][0])/atlas_median[r+1]
        ...
        diff_45long = (atlas_median[r+1] - r_master[r][1])/atlas_median[r+1]
        ...
        diff_85short = (atlas_median[r+1] - r_master[r][2])/atlas_median[r+1]
        ...
        diff_85long = (atlas_median[r+1] - r_master[r][3])/atlas_median[r+1]
...
def boxplots_counties(data, xlab, ylimits, hline, ylabel, titletosave):
    plt.figure()
    plt.boxplot(data)
    plt.xticks(range(1, 6), xlab)#, rotation = 25)
...
boxplots_counties(diff_allRP_percmin_RCP85_short, RP_list[:-1], yaxislimits, 0, "Fractional Difference (-)", "RCP85_short.png")
boxplots_counties(diff_allRP_percmin_RCP85_long, RP_list[:-1], yaxislimits, 0, "Fractional Difference (-)", "RCP85_long.png")
#FS
...
boxplots_counties(FS_allRP_percmin_RCP85_short, RP_list[:-1], yaxislimits_FS, 1, "Climate Factor of Safety (-)", "RCP85_short_FS.png")
boxplots_counties(FS_allRP_percmin_RCP85_long, RP_list[:-1], yaxislimits_FS, 1, "Climate Factor of Safety (-)", "RCP85_long_FS.png")


#%%
r'''
#method to plot maps for each RP
#counties protected from min RCP and time period
CBWcounties = allUScounties[allUScounties['GEOID'].isin(geoid_list)]
for i in range(len(RP_list)-1): #5 RPs
    CBWcounties[RP_list[i]] = diff_allRP_protectmin[i]
    fig, ax = plt.subplots(1, figsize=(10, 6))
    variable = RP_list[i]
    vmin, vmax = 0.0, 1.1
    CBWcounties.plot(column = variable, cmap='Blues', linewidth=0.8, ax=ax, edgecolor='0.8')
    sm = plt.cm.ScalarMappable(cmap='Blues', norm=plt.Normalize(vmin=vmin, vmax=vmax))
    sm._A = [] #empty array for the data range
    cbar = fig.colorbar(sm) #add the colorbar to the figure
    #plt.legend()
    plt.title(RP_list[i])
    plt.savefig(path_to_save + RP_list[i] + "_minprotectionmap.png")
    plt.close()

#counties protected from max RCP and time period
CBWcounties = allUScounties[allUScounties['GEOID'].isin(geoid_list)]
for i in range(len(RP_list)-1): #5 RPs
    CBWcounties[RP_list[i]] = diff_allRP_protectmax[i]
    fig, ax = plt.subplots(1, figsize=(10, 6))
    variable = RP_list[i]
    vmin, vmax = 0.0, 1.1
    CBWcounties.plot(column = variable, cmap= 'Blues', linewidth=0.8, ax=ax, edgecolor='0.8')
    ...
    #plt.legend()
    plt.title(RP_list[i])
    plt.savefig(path_to_save + RP_list[i] + "_maxprotectionmap.png")
    plt.close()

#method for plotting multiple maps at the same time
def multiplemaps(listname, titletosave):
    ...
    for i in range(len(RP_list)-1): #5 RPs
        ...
        #sm = plt.cm.ScalarMappable(cmap='Blues', norm=plt.Normalize(vmin=vmin, vmax=vmax))
        #sm._A = [] #empty array for the data range
        #cbar = fig.colorbar(sm, ax = axs[i]) #add the colorbar to the figure
    plt.savefig(path_to_save + titletosave)
    #plt.close()
...
multiplemaps(diff_allRP_protect_45short, "_protectionmap_RCP45_2020-2070.png") #RCP 4.5 2020-2070
multiplemaps(diff_allRP_protect_45long, "_protectionmap_RCP45_2050-2100.png") #RCP 4.5 2050-2100
multiplemaps(diff_allRP_protect_85short, "_protectionmap_RCP85_2020-2070.png") #RCP 8.5 2020-2070
multiplemaps(diff_allRP_protect_45long, "_protectionmap_RCP85_2050-2100.png") #RCP 8.5 2050-2100

#counties protected from anywhere between 0 and 4 RCPs and time periods
#add them together
diff_allRP_protect = np.array(diff_allRP_protect_45short) + np.array(diff_allRP_protect_45long) + np.array(diff_allRP_protect_85short) + np.array(diff_allRP_protect_85long)
fig, axs = plt.subplots(2, 3, figsize = (12, 8))
axs = axs.flatten()
vmin, vmax = 0.0, 4.0
for i in range(len(RP_list)-1): #6 RPs
    CBWcounties[RP_list[i]] = diff_allRP_protect[i]
    variable = RP_list[i]
    CBWcounties.plot(column = variable, vmin = vmin, vmax = vmax, cmap='Blues', linewidth=0.8, ax=axs[i], edgecolor='0.8')#, label = variable)#, legend = True)
    axs[i].set_title(RP_list[i])
    sm = plt.cm.ScalarMappable(cmap='Blues', norm=plt.Normalize(vmin=vmin, vmax=vmax))
    sm._A = [] #empty array for the data range
    cbar = fig.colorbar(sm, ax = axs[i], ticks = [0, 1, 2, 3, 4]) #add the colorbar to the figure
#plt.legend(bbox_to_anchor=(1.1, 1.05))
plt.savefig(path_to_save + "_protectionmap_allRPs.png")
#plt.close()
r'''